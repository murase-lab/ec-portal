...
export default function URLGenerator() {
    ...
    // ===== Rakuten Gold Server Redirect State =====
    const [storeUrl, setStoreUrl] = useState('')
    const [redirectFileName, setRedirectFileName] = useState('')
    const [autoGeneratedRedirectUrl, setAutoGeneratedRedirectUrl] = useState('')
    const [finalRedirectUrl, setFinalRedirectUrl] = useState('')

    // ===== Load Store URL from localStorage =====
    useEffect(() => {
        const savedStoreUrl = localStorage.getItem('rakutenStoreUrl')
        if (savedStoreUrl) {
            setStoreUrl(savedStoreUrl)
        }
    }, [])

    // ===== Rakuten Gold Server Functions =====
    const saveStoreUrl = (url) => {
        localStorage.setItem('rakutenStoreUrl', url)
    }

    const generateGoldServerRedirectUrl = () => {
        if (!storeUrl.trim() || !redirectFileName.trim()) {
            return ''
        }
        const url = `https://www.rakuten.ne.jp/gold/${storeUrl.trim()}/${redirectFileName.trim()}.html`
        setAutoGeneratedRedirectUrl(url)
        saveStoreUrl(storeUrl.trim())
        return url
    }
...
<body>
    ...
    const downloadRedirectHtml = () => {
        if (!redirectFileName.trim()) {
            setCouponError('ファイル名を入力してください')
            return
        }

        if (!finalRedirectUrl.trim()) {
            setCouponError('最終リダイレクト先URLを入力してください')
            return
        }

        const htmlContent = generateRedirectHtml(finalRedirectUrl.trim())
        const blob = new Blob([htmlContent], { type: 'text/html' })
        const url = URL.createObjectURL(blob)
        const a = document.createElement('a')
        a.href = url
        a.download = `${redirectFileName.trim()}.html`
    ...
    const useGeneratedRedirectUrl = () => {
        if (autoGeneratedRedirectUrl) {
            setRedirectUrl(autoGeneratedRedirectUrl)
        }
    }
    ...
    const generateCouponUrl = () => {
        ...
        // Add &rt=&rd= with redirect URL if redirectUrl is provided
        const finalUrl = redirectUrl.trim()
            ? `${baseUrl}&rt=&rd=${redirectUrl.trim()}`
            : `${baseUrl}&rt=`
                        ...
                        <>
                            ...
                            <section className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-600 mb-1 block">リンク先URL（リダイレクト先）（任意）</label>
                                        <input
                                            type="text"
                                            value={redirectUrl}
                                            ...
                                            placeholder="遷移先URL"
                                        />
                                        <p className="text-[10px] text-slate-400 mt-1">クーポン取得後に遷移するページURL（空欄可）</p>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full border border-slate-300 rounded-lg px-4 py-3 text-sm font-mono"
                                            placeholder="https://image.rakuten.co.jp/店舗ID/cabinet/coupon/2412cp2000.jpg"
                                        />
                                    </div>
                            ...
                            {/* Rakuten Gold Server Redirect */}
                            <section className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-5 shadow-sm border-2 border-amber-200">
                                <h2 className="text-sm font-black uppercase tracking-widest flex items-center gap-2 mb-4">
                                    <span className="text-amber-600">🏆</span> 楽天ゴールドサーバー用リダイレクト設定
                                </h2>

                                <div className="space-y-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-600 mb-1 block">店舗URL</label>
                                        <div className="flex gap-2">
                                            <span className="flex items-center px-3 bg-white border border-slate-300 rounded-lg text-sm text-slate-500">
                                            ...
                                            <input
                                                ...
                                                onBlur={(e) => saveStoreUrl(e.target.value)}
                                                className="flex-1 border border-slate-300 rounded-lg px-4 py-3 text-sm font-mono"
                                                placeholder="店舗ID"
                                            />
                                        </div>
                                        <p className="text-[10px] text-slate-400 mt-1">店舗URLを入力（自動保存されます）</p>
                                    </div>

                                    <div>
                                        <label className="text-xs font-bold text-slate-600 mb-1 block">ファイル名（サブディレクトリ対応）</label>
                                        <div className="flex gap-2">
                                            <input
                                                type="text"
                                                value={redirectFileName}
                                                onChange={(e) => setRedirectFileName(e.target.value)}
                                                className="flex-1 border border-slate-300 rounded-lg px-4 py-3 text-sm font-mono"
                                                placeholder="campaign/winter-sale"
                                            />
                                            <span className="flex items-center px-3 bg-white border border-slate-300 rounded-lg text-sm text-slate-500">
                                                .html
                                            </span>
                                        </div>
                                        <p className="text-[10px] text-slate-400 mt-1">サブディレクトリを含むファイル名を入力（例: campaign/winter-sale）</p>
                                    </div>

                                    <div>
                                        <label className="text-xs font-bold text-slate-600 mb-1 block">最終リダイレクト先URL</label>
                                        <input
                                            type="text"
                                            value={finalRedirectUrl}
                                            onChange={(e) => setFinalRedirectUrl(e.target.value)}
                                            className="w-full border border-slate-300 rounded-lg px-4 py-3 text-sm font-mono"
                                            placeholder="検索結果の絞り込みURL"
                                        />
                                        <p className="text-[10px] text-slate-400 mt-1">HTMLファイルからリダイレクトする最終的な遷移先URL</p>
                                    </div>

                                    <button
                                        onClick={generateGoldServerRedirectUrl}
                                        className="w-full bg-amber-600 text-white py-3 rounded-lg font-bold text-sm hover:bg-amber-700 transition-colors"
                                        disabled={!storeUrl.trim() || !redirectFileName.trim()}
                                    >
                                        <span className="material-symbols-outlined text-sm align-middle mr-1">link</span>
                                        リダイレクト先URLを生成
                                    </button>

                                    {autoGeneratedRedirectUrl && (
                                        <div className="space-y-3">
                                            <div className="bg-white rounded-xl p-4 border border-amber-200">
                                                <div className="flex items-center justify-between mb-2">
                                                    <span className="text-[10px] font-bold text-slate-500 uppercase">生成されたリダイレクト先URL</span>
                                                </div>
                                                <p className="text-sm text-blue-600 font-mono break-all">{autoGeneratedRedirectUrl}</p>
                                            </div>

                                            <div className="grid grid-cols-2 gap-2">
                                                <button
                                                    onClick={useGeneratedRedirectUrl}
                                                    className="bg-blue-600 text-white py-2 rounded-lg font-bold text-xs hover:bg-blue-700 transition-colors"
                                                >
                                                    ↑ リンク先に設定
                                                </button>
                                                <button
                                                    onClick={downloadRedirectHtml}
                                                    className="bg-green-600 text-white py-2 rounded-lg font-bold text-xs hover:bg-green-700 transition-colors"
                                                >
                                            ...
                                            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                                                <p className="text-[10px] text-blue-700 font-bold mb-1">💡 使い方</p>
                                                <ul className="text-[10px] text-blue-600 space-y-1 list-disc list-inside">
                                                    <li>生成されたURLが01の「リンク先URL」になります</li>
                                                    <li>「リンク先に設定」で上のリンク先URLフィールドに自動入力</li>
                                                    <li>「HTMLダウンロード」で最終リダイレクト先へのHTMLファイルを保存</li>
                                                    <li>ダウンロードしたHTMLをゴールドサーバーにアップロード</li>
                                                </ul>
                                            </div>
                                        </div>
                                    )}
